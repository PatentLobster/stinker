<template>
  ...
  <div class="px-4 py-5 sm:px-6  divide-y overflow-y-scroll max-h-full mb-2">
    <div class="mb-2">
      <h2 class="text-lg leading-6 font-medium text-gray-900 dark:text-slate-50">
        {{ command.name }}
      </h2>
      <p class="mt-1 max-w-2xl text-sm text-gray-500 dark:text-slate-200">
        {{ command.description }}
      </p>
    </div>
    <div class="mb-4 divide-y">
      <div class="sm:col-span-6 py-4"
        ...
        <div class="mt-1 flex rounded-md shadow-sm">
                        <span class="inline-flex h-8 items-center px-3 rounded-l-md border border-r-0  sm:text-sm
                        border-blue-gray-300 bg-blue-gray-50 text-blue-gray-500
                        dark:border-slate-300 dark:bg-slate-500 dark:text-slate-200"
                        >
                          {{ arg.name }}
                        </span>
          <input type="text" :name="arg.name" :id="arg.name"
                 ...
                 class="flex-1 block w-full min-w-0 h-8 pl-3 rounded-none rounded-r-md  sm:text-sm
                 border-blue-gray-300 text-blue-gray-900 focus:ring-blue-500 focus:border-blue-500
                 dark:border-slate-300 dark:bg-slate-700 dark:text-slate-200"
          >
        </div>
      </div>
      ...
      <div class="sm:col-span-6 py-4"
        ...
        <div v-if="arg.accept_value === true" class="mt-1 flex rounded-md shadow-sm">
                        <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-blue-gray-300 bg-blue-gray-50 text-blue-gray-500 sm:text-sm dark:border-slate-300 dark:bg-slate-500 dark:text-slate-200">
                          {{ arg.name }}
                        </span>
          <input type="text" :name="arg.name" :id="arg.name"
                 ...
                 @keyup="option_update"
                 class="flex-1 h-8 block w-full min-w-0 border-blue-gray-300 rounded-none rounded-r-md text-blue-gray-900 sm:text-sm focus:ring-blue-500 focus:border-blue-500
dark:border-slate-300 dark:bg-slate-700 dark:text-slate-200">
        </div>
